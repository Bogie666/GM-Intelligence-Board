...
function DomoDatasetCreate({ workspace }: { workspace: ProductionAdminSettingsWorkspace }) {
  ...
  return <section className="production-panel"><div className="production-panel-heading"><div><span>Governed Domo contract</span><h2>Declare a dataset source</h2></div></div>
    <p className="production-boundary-note">The dataset GUID and reduction contract are immutable after approval. Only ready, tenant-scoped Domo connections can be selected.</p>
    <form action={action} className="production-form-grid"><label>Ready connection<select name="connectionId" required defaultValue=""><option value="" disabled>Choose Domo connection</option>{connections.map((connection) => <option key={connection.id} value={connection.id}>{connection.display_name}</option>)}</select></label><label>Dataset GUID<input name="datasetId" required pattern="[0-9A-Fa-f-]{36}" /></label><label>Source name<input name="name" required maxLength={200} /></label><label>Reduction<select name="reduction" value={reduction} onChange={(event) => setReduction(event.target.value)}><option value="sum">Sum</option><option value="average">Average</option><option value="count">Count rows</option><option value="latest">Latest</option></select></label><label>Description<input name="description" maxLength={500} /></label><label>Value column<input name="valueColumn" maxLength={120} required={reduction !== "count"} disabled={reduction === "count"} /></label><label>Date column<input name="dateColumn" maxLength={120} required={reduction === "latest"} /><small>{reduction === "latest" ? "Required: latest is selected chronologically and conflicting timestamp ties fail closed." : "Optional: when supplied, rows are bounded to the observation period."}</small></label><label>Filter column<input name="filterColumn" maxLength={120} /></label><label>Filter value<input name="filterValue" maxLength={200} /></label><div className="production-form-footer"><span>Filter column and value must be supplied together.</span><Submit>Declare Domo dataset</Submit></div></form><Notice state={state} />
  </section>;
}
...
function DomoDatasetRecord({ source, workspace }: { source: ProductionDomoDatasetSource; workspace: ProductionAdminSettingsWorkspace }) {
  ...
  return <article className="production-record">
    ...
    <p>{source.description || "No description."}</p><small>Connection: {connectionLabel} · Value: {source.value_column ?? "row count"} · date: {source.date_column ?? "none"} · filter: {source.filter_column ? `${source.filter_column} = ${source.filter_value}` : "none"}</small>
