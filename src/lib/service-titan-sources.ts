...
export interface ServiceTitanEndpointRecipe {
  ...
  lineage: string;
  /** Historical contracts remain loadable for lineage but cannot be selected for new bindings. */
  retired?: true;
}
...
export const serviceTitanEndpointRecipes: ServiceTitanEndpointRecipe[] = [
  ...
  { id: "completed-revenue", version: 2, name: "Completed revenue (job completion date)", description: "Revenue from jobs completed in the period — uses job completion date and job total to match Lex KPI's completed-revenue basis.", capability: "jobs", outputKind: "currency", defaultRefreshInterval: "1h", allowedRefreshIntervals: ["15m", "30m", "1h", "4h", "24h"], lineage: "Jobs + completion date" },
  ...
  { id: "sales-close-rate", version: 2, name: "Sales close rate (job opportunity basis)", description: "Jobs with at least one Sold estimate above the Sold Threshold divided by total jobs with any estimate — matches ST's close rate definition. Threshold configurable per binding via parameter_values.soldThreshold (default $1.01).", capability: "estimates", outputKind: "percent", defaultRefreshInterval: "1h", allowedRefreshIntervals: ["30m", "1h", "4h", "24h"], lineage: "Estimates + job opportunity grouping" },
  ...
  { id: "completed-job-type-count", version: 2, name: "Completed job-type count", description: "Completed jobs in the period whose jobTypeId is in binding parameter_values.includedJobTypeIds. The IDs are required and malformed or absent values fail closed; set membershipRequired true only for the club-maintenance subset.", capability: "jobs", outputKind: "number", defaultRefreshInterval: "1h", allowedRefreshIntervals: ["30m", "1h", "4h", "24h"], lineage: "Jobs + completion date + binding-pinned job-type IDs" },
  ...
  { id: "average-invoice-ticket", version: 2, name: "Average ticket (completion basis)", description: "Revenue from jobs completed in the period divided by jobs with non-zero total — matches Lex KPI's per-revenue-job average ticket basis.", capability: "jobs", outputKind: "currency", defaultRefreshInterval: "1h", allowedRefreshIntervals: ["30m", "1h", "4h", "24h"], lineage: "Jobs + revenue job count" },
  ...
  { id: "inbound-calls-count", version: 1, name: "Inbound calls", description: "Non-abandoned inbound calls counted for the period.", capability: "call-center", outputKind: "number", defaultRefreshInterval: "30m", allowedRefreshIntervals: ["15m", "30m", "1h", "4h"], lineage: "Call Center + abandonment mapping" },
  { id: "inbound-call-booking-rate", version: 2, name: "Inbound call booking rate", description: "Inbound calls with a job number attached divided by total inbound calls for the period — historical total-inbound basis.", capability: "call-center", outputKind: "percent", defaultRefreshInterval: "30m", allowedRefreshIntervals: ["15m", "30m", "1h", "4h"], lineage: "Call Center + booking attachment", retired: true },
  { id: "inbound-call-booking-rate", version: 3, name: "Inbound call booking rate (qualified leads)", description: "Booked jobs from qualified inbound call leads divided by total qualified inbound call leads. Excludes Excused and NotLead/existing-job calls; qualification uses ServiceTitan call classification, 60-second duration, or Is Lead call reasons.", capability: "call-center", outputKind: "percent", defaultRefreshInterval: "30m", allowedRefreshIntervals: ["15m", "30m", "1h", "4h"], lineage: "Telecom calls + nested leadCall direction, duration, call type, Is Lead reason, and booked job number" },
  { id: "new-memberships", version: 1, name: "New memberships", description: "Customer memberships created during the period, filtered through governed business-unit mappings.", capability: "memberships", outputKind: "number", defaultRefreshInterval: "4h", allowedRefreshIntervals: ["1h", "4h", "24h"], lineage: "Customer memberships + creation window", retired: true },
  { id: "new-memberships", version: 2, name: "New memberships (start-date basis)", description: "Customer memberships whose local membership start date falls inside the observed period, excluding future-dated starts.", capability: "memberships", outputKind: "number", defaultRefreshInterval: "4h", allowedRefreshIntervals: ["1h", "4h", "24h"], lineage: "Complete membership inventory + local start-date window" },
  { id: "canceled-memberships", version: 1, name: "Canceled memberships", description: "Customer memberships whose cancellation date falls inside the period.", capability: "memberships", outputKind: "number", defaultRefreshInterval: "4h", allowedRefreshIntervals: ["1h", "4h", "24h"], lineage: "Customer memberships + cancellation window", retired: true },
  { id: "canceled-memberships", version: 2, name: "Membership cancellations + expirations", description: "Customer memberships whose earliest cancellation or expiration date falls inside the local observed period, excluding future-dated ends.", capability: "memberships", outputKind: "number", defaultRefreshInterval: "4h", allowedRefreshIntervals: ["1h", "4h", "24h"], lineage: "Complete membership inventory + local effective-end window" },
  { id: "membership-net-growth", version: 1, name: "Membership net growth", description: "Memberships created minus memberships canceled or expired (effective end) during the period.", capability: "memberships", outputKind: "number", defaultRefreshInterval: "4h", allowedRefreshIntervals: ["1h", "4h", "24h"], lineage: "Customer memberships + creation and effective-end windows", retired: true },
  { id: "membership-net-growth", version: 2, name: "Membership net growth (event-date basis)", description: "Membership starts minus effective ends inside the local observed period. Effective end is the earliest cancellation or expiration date; future events are excluded.", capability: "memberships", outputKind: "number", defaultRefreshInterval: "4h", allowedRefreshIntervals: ["1h", "4h", "24h"], lineage: "Complete membership inventory + local start/effective-end windows" },
  { id: "sold-estimates-value", version: 1, name: "Sold estimates value", description: "Subtotal of estimates sold during the period, filtered through governed business-unit mappings.", capability: "estimates", outputKind: "currency", defaultRefreshInterval: "1h", allowedRefreshIntervals: ["30m", "1h", "4h", "24h"], lineage: "Estimates + sold window", retired: true },
  { id: "sold-estimates-value", version: 2, name: "Sold + scheduled pipeline", description: "Canonical sold estimate subtotal for active fulfillment jobs scheduled to finish after as-of and by local month-end. Requires an approved non-empty business-unit scope and deduplicates shared sold scopes.", capability: "jobs", outputKind: "currency", defaultRefreshInterval: "1h", allowedRefreshIntervals: ["30m", "1h", "4h", "24h"], lineage: "Jobs + final appointment + referenced sold estimate + business-unit mapping" },
  { id: "sales-opportunity-count", version: 1, name: "Sales opportunity count", description: "Distinct non-null estimate jobId values in the created-period cohort, using the same business-unit filtering and opportunity denominator semantics as Sales Close Rate v2.", capability: "estimates", outputKind: "number", defaultRefreshInterval: "1h", allowedRefreshIntervals: ["30m", "1h", "4h", "24h"], lineage: "Estimates + created-period distinct job opportunities" },
  { id: "sold-estimate-average-ticket", version: 1, name: "Sales average ticket (sold estimate record)", description: "Sold estimate records with soldOn in the period: subtotal sum divided by sold-record count. This is an estimate-record contract, not a canonical-winning-opportunity contract, and zero sold records fail closed.", capability: "estimates", outputKind: "currency", defaultRefreshInterval: "1h", allowedRefreshIntervals: ["30m", "1h", "4h", "24h"], lineage: "Sold estimates + sold window + subtotal / sold-record count" },
  { id: "jobs-with-appointments-count", version: 1, name: "Jobs with appointments", description: "Jobs with an appointment starting during the period, filtered through governed business-unit mappings.", capability: "jobs", outputKind: "number", defaultRefreshInterval: "1h", allowedRefreshIntervals: ["30m", "1h", "4h", "24h"], lineage: "Jobs + appointment window" },
];

/** Recipes eligible for new UI selections; retired versions remain available for historical lineage. */
export const selectableServiceTitanEndpointRecipes = serviceTitanEndpointRecipes.filter((recipe) => recipe.retired !== true);
