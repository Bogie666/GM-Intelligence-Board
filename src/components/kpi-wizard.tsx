...
const typeOptions: { id: CustomKpiDefinition["type"]; title: string; copy: string; icon: typeof Layers3 }[] = [
  ...
  { id: "external", title: "External KPI", copy: "Model Domo, GA4, GBP, call-system, finance, or another governed connector.", icon: Database },
];
    ...
    <div className="wizard-body">
      <div className="wizard-main">
        ...
        {currentStep === "source" && <div className="wizard-panel"><div className="wizard-panel-title"><span>Step 3</span><h3>Configure data lineage</h3><p>The selected KPI type controls which source settings are allowed.</p></div>
          ...
          {draft.type === "external" && <div className="wizard-form-grid"><label>External provider<select value={draft.provider ?? ""} onChange={(event)=>{const provider=event.target.value as CustomKpiDefinition["provider"];patch({provider,externalDatasetId:provider === "Domo" ? draft.externalDatasetId : undefined})}}><option value="">Select provider…</option><option>Domo</option><option>GA4</option><option>Google Business Profile</option><option>Call System</option><option>Finance</option><option>Other</option></select></label>{draft.provider === "Domo" && <label>Domo dataset ID<input value={draft.externalDatasetId ?? ""} onChange={(event)=>patch({externalDatasetId:event.target.value})} placeholder="Approved dataset GUID" aria-invalid={issues.some((i)=>i.code==="domo-dataset")} /></label>}<label>Metric / column key<input value={draft.externalMetricKey ?? ""} onChange={(event)=>patch({externalMetricKey:event.target.value})} placeholder={draft.provider === "Domo" ? "e.g. revenue_actual" : "e.g. reviews.new_5_star"} /></label><label>Expected refresh<select value={draft.refreshCadence} onChange={(event)=>patch({refreshCadence:event.target.value as CustomKpiDefinition["refreshCadence"]})}><option value="daily">Daily</option><option value="weekly">Weekly</option><option value="monthly">Monthly</option><option value="ad-hoc">Ad hoc</option></select></label><label>Stale after hours<input type="number" min="1" value={draft.staleAfterHours ?? ""} onChange={(event)=>patch({staleAfterHours:numericValue(event.target.value)})}/></label><div className="lineage-note warning span-two"><CircleAlert/><div><strong>{draft.provider === "Domo" ? "Domo framework ready; materialization pending" : "Connector not active in this test build"}</strong><p>{draft.provider === "Domo" ? "OAuth, allowlisted metadata access, and CSV export are server-side. A governed field mapping and scheduled snapshot are still required before this KPI can use live Domo values." : "A manual demo snapshot is required for preview and will be labeled as such. Missing external data is never converted to zero."}</p></div></div></div>}
        </div>}
